...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    elif self.car_fingerprint in [CAR.OUTBACK, CAR.LEGACY, CAR.OUTBACK_2023]:
    ...
    if self.car_fingerprint in [CAR.OUTBACK, CAR.LEGACY, CAR.OUTBACK_2023]:
  ...
  def get_can_parser(CP):
    ...
    if CP.carFingerprint not in [CAR.OUTBACK, CAR.LEGACY, CAR.OUTBACK_2023]:
      signals += [
        ("FL", "Wheel_Speeds", 0),
        ("FR", "Wheel_Speeds", 0),
        ("RL", "Wheel_Speeds", 0),
    ...
    if CP.carFingerprint not in [CAR.OUTBACK, CAR.LEGACY, CAR.CROSSTREK_2020H, CAR.OUTBACK_2023]:
  ...
  def get_body_can_parser(CP):
    ...
    elif CP.carFingerprint in [CAR.OUTBACK, CAR.LEGACY, CAR.OUTBACK_2023]:
      signals += [
  ...
  def get_cam_can_parser(CP):
    if CP.carFingerprint in PREGLOBAL_CARS:
      ...
      if CP.carFingerprint not in [CAR.CROSSTREK_2020H, CAR.OUTBACK, CAR.LEGACY, CAR.OUTBACK_2023]:
        signals += [
          ("Counter", "ES_Distance", 0),
          ("Signal1", "ES_Distance", 0),
          ("Cruise_Fault", "ES_Distance", 0),
